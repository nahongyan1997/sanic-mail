...
class SanicMail:
    ...
    async def smtp_conn(self, app: Sanic) -> None:
        ...
        await self.smtp.login(
            current_app.config.MAIL_SENDER, current_app.config.MAIL_SENDER_PASSWORD
        )

    async def smtp_connection(self, app: Sanic) -> None:
        await self.smtp_conn(app)
        logger.info("[SanicMail] smtp connected !")

    async def smtp_close(self, app: Sanic) -> None:
    ...
    async def send_email(
        ...
        if self.smtp:
            if not self.smtp.is_connected:
                await self.smtp_conn(current_app)
                logger.info("[SanicMail] smtp reconnected !")
